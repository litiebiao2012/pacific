...
import com.pacific.common.exception.UtilException;
...
public class DateUtil {
    ...
    public static boolean checkDateArrayHasIntersection(String[] begins, String[] ends) {
        Assert.notNull(begins);
        Assert.notNull(ends);
        if (begins.length != ends.length) PacificException.throwEx("length not equals!");
        boolean flag = false;
        ...
        for (String begin : begins) {
            ...
            if (beginStr.length != 2) PacificException.throwEx("date error!");

            int hour = Integer.parseInt(beginStr[0]);
            int millis = Integer.parseInt(beginStr[1]);
            DateTime beginDateTime = new DateTime(dateNow.getYear(), dateNow.getMonthOfYear(), dateNow.getDayOfMonth(), hour, millis);

            String[] endStr = ends[index].split("\\:");
            if (endStr.length != 2) PacificException.throwEx("date error!");

            int endMinutes = Integer.parseInt(endStr[0]);
            int endSeconds = Integer.parseInt(endStr[1]);
            DateTime endDateTime = new DateTime(dateNow.getYear(), dateNow.getMonthOfYear(), dateNow.getDayOfMonth(), endMinutes, endSeconds);

            TimeRange timeRange = new TimeRange();
            timeRange.setBeginTime(beginDateTime);
            timeRange.setEndTime(endDateTime);
            timeRangeList.add(timeRange);
            index++;
        }

        for (TimeRange trOne : timeRangeList) {
            for (TimeRange trTwo : timeRangeList) {
                if (trTwo.getBeginTime().isAfter(trOne.getBeginTime()) && trTwo.getBeginTime().isBefore(trOne.getEndTime())) {
                    flag = true;
                    return flag;
                }

                if (trTwo.getEndTime().isAfter(trOne.getBeginTime()) && trTwo.getEndTime().isBefore(trOne.getEndTime())) {
                    flag = true;
                    return flag;
                }
            }
    ...
    public static void main(String args[]) {
        String[] begins = new String[] { "10:12", "1:00" };
        String[] ends = new String[] { "12:00", "1:21" };

        boolean flag = checkDateArrayHasIntersection(begins, ends);
     ...
     * @return
     */
    public static Calendar parseForSimpleCal(String dateStr) {
        return parseCal(simple_format, dateStr);
    }
    private static Calendar parseCal(SimpleDateFormat simpleDateFormat, String dataStr) {
        try {
     ...
     * @return
     */
    public static Date getSevenDayBegin() {
        Calendar calendar = Calendar.getInstance();
